...
export const hashEmail = async (email: string): Promise<string> => {
  const secret = (typeof SECRET_KEY_ENV === 'string' && SECRET_KEY_ENV) || (typeof SECRET_KEY === 'string' && SECRET_KEY);
  if (!secret) {
    return '0'.repeat(64);
  }
...
export const isSessionValid = (): boolean => {
  try {
    const verificationTime = localStorage.getItem(STORAGE_KEYS.VERIFICATION_TIME);
...
export const isCurrentlyVerified = (): boolean => {
  try {
    ...
    if (localEmail && localToken && isSessionValid()) {
      if (sessionEmail && sessionToken && localEmail === sessionEmail && localToken === sessionToken) {
        return true;
      }
